...
## 2026-05-08 — Raise LOC ceiling to 16,850 for BRIEFS-V1 parser/matcher
**Decision:** Raise `tests/test_loc_budget.py` `TOTAL_LIMIT` from 16,650 → 16,850. Per-file 600 LOC unchanged.
**Why:** BRIEFS-V1 starts a new product surface: cheap focused evidence briefs derived from certified full papers. Phase 1-2 adds `agent/briefs/question_parser.py` and `agent/briefs/topic_matcher.py`, converting a user question into a structured BriefQuery and matching interventions to topic-pack aliases. Keeping this in `agent/briefs` avoids burying product logic in scripts and keeps later filter/writer phases on a coherent package boundary.
...
**Revisit if:** BRIEFS-V1 Phase 3-5 add more runtime modules; each phase must justify its own measured budget or delete equivalent dead weight.
...
**Alternatives rejected:**
- Raise to 10,500 (just enough for Phase 2/3) — rejected; the same tight-headroom dynamic that just bit Day 10.16 (commit at 9,997/10,000) would replay immediately when Phase 2 lands and force a second ceiling raise inside one day, which is exactly the "raise the ceiling six times" red flag the reviewers flagged. One raise with 2,000 headroom is more disciplined than two raises with 500 each.
- Refactor before raising — investigated; the top files (synthesis_writer 565, orchestrator 549, citation_trace 527, synthesis 525) are all well-scoped single-responsibility modules. There is no obvious 200-cloc deletion target without cosmetic surgery on docstrings that document past trust-spine reasoning. Net deletion-only path costs more time than it saves.
- Defer Phase 2/3 — rejected; Phase 1.5's empirical re-audit of the existing artifact (10.0 → 3.75) shows Phase 1's validators correctly catch the bugs they were built for, but the system still has two architectural gaps (tension matrix, citation format) flagged by every external reviewer. Shipping Phase 1+1.5 alone leaves the artifact at "honest fail" rather than "honest pass," which doesn't move the user's deliverable forward.
**Revisit if:** runtime LOC approaches 11,500 without all three Day 10.17 phases shipped + a writer-side hedge-discipline pass. The user-quote constraint is "if needed and not bloat" — a future ceiling raise must show the same discriminating-test coverage and post-fix audit improvement that Phase 1+1.5 just demonstrated.
