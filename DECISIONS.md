...
## 2026-04-24 — MoA+Spar default + draft-quality validators
**Decision:** Make MoA+Spar the default model path for extraction and drafting, and add deterministic draft-quality checks plus per-source evidence-object fields.
**Why:** Recent bridge tests showed prompt-only iteration was trading one failure class for another: citation wiring, raw extraction leaks, weak source hierarchy, and conclusion contradictions. The scalable fix is a model-review bridge plus structural validators, not topic-specific patches.
**What shipped:**
- `agent/cli.py` and `agent/extractor.py` now wrap real `MimoClient` providers with `MoaSparBridgeClient`; tests can still inject fake providers without external calls.
- `agent/moa_spar_bridge.py` now accepts an existing builder and treats malformed reviewer/judge JSON as a material rejection with one bounded fix pass instead of crashing.
- `agent/drafter.py` now annotates every retained source with `evidence_tier`, `strict_eligibility_met`, `evidence_confidence`, and `risk_of_bias` using generic evidence signals, including a non-biomedical strict-result test.
- Markdown now includes a standard Evidence Table and MoA+Spar reasoning stamp when bridge metadata is present.
- `agent/validator.py` now logs draft-quality violations for missing Key Findings citations, raw extraction leaks in Abstract/Conclusion, and same-citation positive-vs-null conclusion contradictions.
**Budget impact:** Runtime is now ~4,210 LOC by `find agent -name '*.py' -print0 | xargs -0 wc -l | tail -1`; raise hard ceiling to 4,500 until the next deletion pass.
**Alternatives rejected:**
- Hardcode metformin/rapamycin exclusions — rejected; the shipped checks are role/signal based.
- Let malformed Spar review JSON fail the run — rejected; one bounded fix pass is cheaper and safer.
- Keep MoA+Spar as optional/manual — rejected; the user requested it as the default quality path.
**Revisit if:** live bridge latency or rate limits are unacceptable, at which point add caching or a documented degraded mode rather than changing source/tier rules.
