...
## 2026-04-25 — Replace MiniMax/DeepSeek bridge slots with OpenRouter free models
**Decision:** Keep MiMo v2 Pro as the builder/synthesizer, but move the optional MoA/Spar reviewer slot to OpenRouter `nvidia/nemotron-3-super-120b-a12b:free` and the judge slot to `google/gemma-4-31b-it:free`.
**Why:** The optional bridge needs non-Xiaomi adjudication diversity without MiniMax subscription or DeepSeek pricing exposure. OpenRouter currently lists both target slugs as free with 262K context, and its chat API supports OpenAI-compatible JSON mode.
...
**Revisit if:** either free endpoint disappears, JSON mode fails in live probes, or the gold corpus shows more than a 10% quality regression.
