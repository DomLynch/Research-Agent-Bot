...
def _build_grok_prompt(
    ...
    system = (
        ...
        "issues pass. These rules are TOPIC-AGNOSTIC and apply to any "
        "research domain (biomedical, climate, materials, economics, "
        "social science, tech, etc.):\n"
        "   (a) Contradictions between sections (Abstract vs Results, "
        "Discussion vs Conclusion, Limitations vs claims).\n"
        "   (b) Subtle reasoning gaps — flag whenever the prose "
        "extrapolates beyond what the cited receipt supports:\n"
        "       • Mechanism-level evidence (in vitro, model system, "
        "computational) used as load-bearing support for a claim "
        "about the target real-world population/system, without a "
        "direct-target citation in the same sentence.\n"
        "       • Surrogate / proxy variable presented as if "
        "equivalent to the primary outcome of interest. (E.g. a "
        "biomarker in clinical research, a leading indicator in "
        "economics, an in-silico metric in materials/engineering.)\n"
        "       • Sub-population, model-system, or scoped-context "
        "evidence used to support a claim about a broader population "
        "or unscoped context, without an explicit hedge naming the "
        "scope ('in <named population/model/regime>').\n"
        "       • Effect-direction mismatch between the cited receipt's "
        "claim and the prose summary (the receipt says 'no effect' "
        "or 'mixed', the prose says 'shows an effect').\n"
        "       • Causation language ('reduces', 'prevents', 'causes', "
        "'drives', 'leads to') used where only association / "
        "correlation / co-occurrence is actually supported. Whether "
        "the underlying study is an RCT, instrumental-variable "
        "analysis, natural experiment, or just an observational "
        "cohort drives what causal language is admissible.\n"
        "       • Aggregation across heterogeneous receipts presented "
        "as if pooled, when no formal pooled analysis was actually "
        "run. Phrases like 'studies show', 'evidence indicates', "
        "'research demonstrates' must be either (i) anchored to one "
        "receipt or (ii) accompanied by an explicit pool result.\n"
        "       • Generalisation across scope dimensions (time period, "
        "geography, population, dose, regime, scale) without "
        "acknowledgement of the scope limit.\n"
        "   (c) Lenient hedge-words. Be STRICT here — flag any of:\n"
        "       • 'may' / 'might' / 'could' / 'suggests' WITHOUT a "
        "supporting receipt cited in the same sentence (a hedge "
        "without an anchor is rhetorical filler).\n"
        "       • 'is associated with' applied to a study where the "
        "receipt establishes a different relationship (e.g. a "
        "mechanism study should say 'modulates'/'affects'; an "
        "experiment should say 'produces'/'yields'; only "
        "population-level correlational data warrants 'associated').\n"
        "       • 'demonstrates', 'confirms', 'establishes', 'proves' "
        "— these are stronger than 'suggests'/'indicates' and "
        "require the receipt to be the strongest study type the "
        "field accepts as definitive (e.g. RCT or meta-analysis in "
        "clinical research; replicated multi-site experiment in "
        "physical sciences; pre-registered RCT or quasi-experiment "
        "with strong identification in social science).\n"
        ...
        "Either commit to the claim with a single hedge, or drop "
        "the claim.\n"
        "       • Domain-shift hedges that hide the actual issue. If "
        "the receipt is in domain X and the claim is about domain Y, "
        "the prose must name the gap ('extrapolating from X to Y'), "
        "not bury it under generic 'may'.\n"
