...
def _classification_criteria_lines() -> list[str]:
    return [
        ...
        "- **Directional signal** is counted within the assigned outcome class only. Positive/negative labels describe reported directional findings, not proof of statistical significance; a comparative point estimate without uncertainty remains descriptive. Mixed includes changed and null endpoints and does not establish overall benefit. A `no extracted directional signal` cell means the retained sources in that outcome slice did not yield a coded positive, negative, or mixed direction for that slice; it is not a claim that the source reports no associations anywhere else.",
        "- **Evidence tier** records a source-design category, not a risk-of-bias judgment or certainty rating. An A1 label alone does not establish hard endpoints or a decision-grade conclusion; these require source-specific appraisal. The prose writer cannot change frozen source classifications.",
        "",
    ]
