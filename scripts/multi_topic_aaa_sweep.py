...
def _run_topic(topic: str, *, passes: int, label: str, started: str) -> dict:
    log_path = REPO / "logs" / f"aaa_sweep_{started}_{topic}.log"
    result: dict = {"passes": [], "log_path": str(log_path.relative_to(REPO))}
    for n in range(1, passes + 1):
        ts = dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%dT%H-%M-%SZ")
        out_dir = REPO / "runs" / f"synthesis-{topic}-v06-{label}-pass{n}-{ts}"
        ...
        if rc == 0 and (out_dir / "full_paper.md").exists():
            rc_q = _run(
                ["python3", "scripts/paper_quality_runtime.py", str(out_dir)],
                cwd=REPO,
                log_path=log_path,
            )
    ...
    if len(result["passes"]) >= 2:
        result["l6_classification"] = _classify_l6(
            result["passes"][0]["gates"],
            result["passes"][1]["gates"],
        )
    return result
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="10-topic × 2-pass AAA sweep.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Parallel topic workers; each topic still runs passes in order.",
    )
    ...
    jobs = max(1, min(args.jobs, len(args.topics) or 1))
    with ThreadPoolExecutor(max_workers=jobs) as pool:
        ...
        for future in as_completed(futures):
            ...
            summary["results"][topic] = future.result()
            _write_summary(summary_path, summary)
