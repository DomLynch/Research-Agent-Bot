...
def repair_revision_surface(
    ...
    if tension_asks:
        ...
        if paragraph and section and paragraph not in section.group("body"):
            ...
            text, n = re.subn(rf"(^## {target}\b[^\n]*\n)",
                              lambda match: cast(re.Match[str], match).group(1) + "\n" + paragraph + "\n", text, count=1, flags=re.M)
            if n:
    ...
    if coverage.asks_mechanistic_content_reconciliation(feedback):
        ...
        text, n = re.subn(r"(?P<lead>[ \t]*)[^.\n]*\bno sources classified primarily as mechanistic\b[^.\n]*\.",
                          lambda match: cast(re.Match[str], match).group("lead") + replacement, text, count=1, flags=re.I)
        if n:
...
def _dedupe_repeated_blocks(text: str) -> tuple[str, int]:
    ...
    for i in range(0, len(chunks), 2):
        ...
        if not norm:
            continue
        if re.match(r"\*\*\s*(?:thesis|resolution\s+criteria)\s*:", norm, flags=re.I) or norm.lower().startswith("outcome-class coded-direction reconciliation:") or norm.startswith(("Selection assessment (", "| Selection gate |")):
            continue
