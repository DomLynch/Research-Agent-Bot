...
_USER = (
    ...
    "A JSON revision with claim_id, claim, status, sources, passages_considered and required_check is a structured claim-review finding, not a request to add manuscript sections. For it, verify the manuscript claim against the complete named source evidence and the finding's passages/comparisons. INSUFFICIENT_SOURCE_TEXT is not affirmative evidence; author scope/process claims require the supplied author context. Mark false for a real unsupported or uncorrected claim, not because words quoted in the finding imply an unrelated section requirement. "
    ...
    "A revision_feedback review_reference to the numbered asks means those complete asks reproduce the feedback. A source_partition divides only its named source field into ordered text parts; all other fields are complete. Judge the evidence present in this part, and do not mark an ask false solely because another part of that field is absent. Every part must pass.\n"
    "REQUIRED REVISIONS:\n{asks}\n\n=== MANUSCRIPT ===\n{paper}"
    "\n\n=== SOURCE EVIDENCE ===\n{evidence}"
    ...
    *,
    ...
    try:
        ...
        for indexes, content in inputs:
            ...
            if not isinstance(batch, list) or len(batch) != len(indexes) or any(type(flag) is not bool for flag in batch):
                return clean
            for index, value in zip(indexes, batch, strict=True):
                flags[index] = flags[index] and value
