...
def render_methods(c: RunModeContract) -> str:
    ...
    return (
        f"## Methods\n\n"
        f"This synthesis used a predeclared corpus of "
        f"{c.n_papers_in_corpus} source papers on {c.topic}. Source "
        f"documents were screened for quantitative outcome statements; "
        f"{c.n_high_confidence_claims_used_by_writer} source-bound "
        f"observations were retained after endpoint, unit, study-label, "
        f"and citation checks.\n\n"
        f"Evidence was grouped by outcome class, study design, direction "
        f"of effect, directness, and endpoint proximity. Cross-paper "
        f"tensions were retained when two findings addressed related "
        f"outcomes but differed in direction, population, comparator, "
        f"measurement method, or follow-up window.\n\n"
        f"Public prose was constrained to the retained evidence set. "
        f"Numeric statements were checked against the source-bound claim "
        f"table, and rows with unresolved endpoint, unit, study-label, or "
        f"citation problems were excluded from the public quantitative "
        f"evidence index.\n\n"
        f"Clinical, observational, review, and mechanistic findings were "
        f"interpreted according to their design limits. Direct human "
        f"trials carried the most weight for clinical endpoints, whereas "
        f"cellular, animal, or tissue-level findings were used to clarify "
        f"plausible mechanisms and boundary conditions rather than to "
        f"establish clinical benefit.\n\n"
        f"The journal main text reports the bounded synthesis rather than "
        f"the full extraction ledger. Dense numeric rows, per-study endpoint "
        f"tables, excluded records, and audit records are preserved in the "
        f"supplement so the main manuscript stays readable while the evidence "
        f"trail remains reproducible.\n\n"
        f"Where endpoints, comparators, or follow-up windows were too "
        f"heterogeneous for pooling, findings were reported as boundary "
        f"conditions instead of pooled treatment effects. This rule prevents "
        f"mechanistic plausibility, biomarker movement, functional response, "
        f"and hard clinical inference from being treated as interchangeable. "
        f"Section placement was controlled by a fixed journal skeleton so "
        f"background context, analytic methods, outcome findings, synthesis, "
        f"limitations, and conclusion remained separate. After assembly, the "
        f"main text was rechecked for section order, minimum section depth, "
        f"citation role, numeric traceability, and public-surface residue. "
        f"This preserves a reproducible boundary between evidence handling "
        f"and manuscript interpretation while avoiding claims about procedures "
        f"that were not part of the run.\n"
    )
