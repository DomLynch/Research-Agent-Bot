# Granite Arbitration Summary — 2026-05-08

## Status

- Pipeline wire: live-gated behind `GRANITE_ARBITRATOR_ENABLED=1`.
- Model: `ibm-granite/granite-4.1-8b` via OpenRouter-compatible API.
...
- Live tuned prompt smoke: 3/10 agreement; improved deletion handling, still over-decides.

## Safety Decision

Granite is not trusted as a free semantic judge. The pipeline wrapper now permits:

- `APPLY` only for exact-after public-surface patches or exact deletion patches.
- No non-empty claim/numeric/structure rewrites from Granite.
...
Granite adds value as a logged third-reviewer signal and can resolve narrow public-surface/deletion disputes. It is not yet strong enough to replace deterministic gates or certify semantic rewrites.
