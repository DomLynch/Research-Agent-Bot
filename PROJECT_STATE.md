...
- `writer.py` 437, `spar.py` 397, `citation_trace.py` 355, `fact_extractor.py` 327 — all over soft cap; all under 600 hard cap. Each carries load-bearing prompts and/or trust-spine gates (claim-binding / cite-subset / verb-ban / p-value / alias / trace-gate). Splitting would couple tightly-related logic.
...
- `writer.py` (Day 4.3 + 4.3-fix — 437 cloc — claim-graph-gated prose drafter. `write_paper(graph, items, traces, spar, *, pack, topic, chain, ledger)` routes by SPAR verdict: `accept_clean` / `accept_caveated` invoke the LLM and gate every sentence; `reject_*` / any `gate_override` skip the LLM and render a structured rejection. **CODE DISPOSES** via SEVEN gates per sentence (4.3-fix added the first three — strict claim-binding contract closes the V1.1 hole where the LLM could cite a valid `[N]` while making a novel claim outside the graph): (1) `claim_ids` non-empty + all in graph (LLM declares which claim the sentence asserts), (2) `text` contains ≥1 `[N]` cite (no uncited claim-of-fact), (3) every cite is in the union of declared claims' supporting_refs (cite-to-claim subset), (4) per-cited-ref `check_verb_ban`, (5) per-cited-ref `check_p_value_in_source`, (6) `check_alias_drift` on the whole text, (7) non-string claim_ids reject (no silent coercion). Each sentence is now an OBJECT `{"claim_ids": [...], "text": "..."}` per the new prompt contract. Failed sentences become `WriterRejection` records. `_spar_banner` renders the verdict prominently — `gate_override` gets TRUST-SPINE TRACE GATE TRIGGERED + canonical AND panel verdicts + failed_trace_count + rationale. References section includes ONLY refs actually cited in kept prose. PROMPT_VERSION = "writer/2026-04-28".)
