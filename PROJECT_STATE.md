...
| _next_    | 2026-04-29 | Day 10.11 empirical: 27 clusters, 1 accepted (filter drops objective spans pre-extraction; cluster count 30→27) but SPAR pass rate stays ~4% — dominant rejection mode shifts to mechanism inflation. Day 10.12 needs mechanism-inflation guards. |
...
| **10.12** (next) | mechanism-inflation guards: detect extrapolation patterns ("protective effect", "may improve", "demonstrated efficacy") in extracted quotes when the cited evidence is mechanistic / single-cell / preclinical, and reject upstream of SPAR | ☑ this slice |
| **10.13** (stretch) | external human review of prose quality — outside automated audit | ☐ |
