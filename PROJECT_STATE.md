...
## Status — 2026-04-28 — Day 4.3 writer shipped (claim-graph-gated prose + gate_override prominence); Day 4.4 planted-failures-through-SPAR next
...
**Tests:** 545/545 passing in 0.41s. ruff clean. git diff --check clean.
**Runtime LOC (cloc-style, the canonical count enforced by `tests/test_loc_budget.py`):** 5,032 / **5,500** ceiling (8.5% headroom; ceiling raised 2026-04-28 — see DECISIONS.md).

**Per-file (cloc-style, soft cap 300, hard cap 600):**
- `spar.py` 397, `writer.py` 376, `citation_trace.py` 355, `fact_extractor.py` 327 — all over soft cap; all under 600 hard cap. Each carries load-bearing prompts and/or trust-spine gates (verb-ban / p-value / alias / cite-in-bundle / trace-gate). Splitting would couple tightly-related logic.
...
- `writer.py` (Day 4.3 — 376 cloc — claim-graph-gated prose drafter. `write_paper(graph, items, traces, spar, *, pack, topic, chain, ledger)` routes by SPAR verdict: `accept_clean` / `accept_caveated` invoke the LLM and gate each sentence; `reject_majority` / `reject_critical` / any `gate_override` skip the LLM and render a structured rejection notice. **CODE DISPOSES** via four sentence-level gates: (1) cite-in-bundle (every `[N]` must be a supporting_ref of a real claim), (2) per-cited-ref `check_verb_ban`, (3) per-cited-ref `check_p_value_in_source`, (4) `check_alias_drift` on the whole sentence. Failed sentences become `WriterRejection` records (drop-and-record, no re-prompt). `_spar_banner` renders the SPAR verdict prominently — `gate_override` gets the most prominent treatment per the reviewer audit-trail requirement: TRUST-SPINE TRACE GATE TRIGGERED header + canonical and panel verdicts both visible + failed_trace_count + gate rationale. References section includes ONLY refs actually cited in the kept prose. PROMPT_VERSION = "writer/2026-04-28".)
...
| 4 | `thesis_tournament.py` + `spar.py` + writer prompt with quality-bar block + judge checklist. Plant-corpus prompt iteration. `gap_analysis.py` only if time permits (non-gating). | All 5 planted failures caught; thesis tournament selects defensible thesis on real corpus. | **PARTIAL** — 4.1 `49677b2` (thesis_tournament) + 4.2 `a1dfcf4` (SPAR) + 4.2-fix `bc5e080` (trust-spine trace gate + GateOverride schema) + 4-prep `dd73dd7` (ceiling raise) + **4.3 writer** (this slice — `writer.py` 376 cloc, claim-graph-gated prose with 4 sentence gates, SPAR verdict routing (accept_* invokes LLM; reject_* / gate_override skip LLM and render rejection notice), gate_override displayed PROMINENTLY per reviewer audit-trail requirement; 26 mock tests, 545/545 total); 4.4 planted-failures-through-SPAR ☐ pending |
