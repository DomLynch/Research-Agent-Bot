...
TOTAL_LIMIT = 24100  # Wave 59 (2026-05-15): +50 for Slice 28 — new finalizer Phase H (`_phase_h_topic_slug_normalise`) substitutes a snake_case or special-cased topic slug (`vitamin_d` / `glp1` / `caloric_restriction` / `omega3`) with the topic pack's `aliases_display[0]` canonical display form (`vitamin D` / `GLP-1` / `caloric restriction` / `omega-3`). Closes the universal `topic_slug_artifact` surface flag (5 instances across the fresh vitamin_d + glp1 runs). Precision-gated by the gate's `_PUBLIC_SLUG_RE.fullmatch` so plain English-word slugs (senolytics/rapamycin/aspirin) are correctly skipped — substituting them would damage prose. Backtick spans (file refs like ``topic_packs/glp1.toml``) are preserved. Universal — no per-topic logic. Wave 58 (2026-05-15): +50 for Slice 19 Phase G surface re-evaluation. Wave 57 (2026-05-14): +50 for Slice 18 prose polish + accountability split + initial Phase G sidecar reconciliation.
