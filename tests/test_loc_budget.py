...
Wave 10 — BRIEFS-V1 Phase 1-4 (16,650 → 17,200):
agent/briefs/question_parser.py (~150 cloc) and topic_matcher.py
(~42 cloc), receipt_filter.py (~90 cloc), brief_writer.py (~122
cloc), plus package exports. Adds schema-first question parsing,
...
TOTAL_LIMIT = 24200  # Wave 61 (2026-05-15): +50 for Slice 31 — thin-corpus product-type downshift. New `thin_corpus_brief` review_type token + `downshift_review_type_for_thin_corpus(declared, n_receipts, n_tensions)` helper in `agent/review_type.py`. Pipeline calls the downshift at manifest-finalisation time so when n_receipts<10 OR n_tensions=0 (universal thresholds — THIN_CORPUS_MIN_RECEIPTS / THIN_CORPUS_MIN_TENSIONS) the run's review_type becomes `thin_corpus_brief`, flowing through Methods rendering, the gate's type-consistency check, and the maturity ladder. Reviewer guidance: do not render a 7k-word manuscript from 2 receipts; produce an honest evidence brief instead. Universal — no per-topic logic. Wave 60 (2026-05-15): +50 for Slice 29 readiness-contract multi-item refresh. Wave 59 (2026-05-15): +50 for Slice 28 Phase H topic-slug normalisation. Wave 58 (2026-05-15): +50 for Slice 19 Phase G surface re-evaluation. Wave 57 (2026-05-14): +50 for Slice 18 prose polish + accountability split + initial Phase G sidecar reconciliation.
