...
TOTAL_LIMIT = 24050  # Wave 58 (2026-05-15): +50 for Slice 19 — Phase G now re-evaluates `journal_surface_gate` against the post-finalizer paper and rewrites `full_paper.journal_surface.json` (caloric_restriction AAA run smoke-test revealed that the pipeline's Stage-5 surface gate runs BEFORE finalizer Phase A swaps in PRISMA-ScR Methods, producing 11 false-positive `methods_pack_incomplete` issues per run, universally). Re-eval reconstructs `animal_citations` + `citation_outcome_map` from on-disk sidecars (`evidence_lanes.json`, `manifest.json`, `citation_registry.json`); existing Phase G reconciliation then propagates the fresh surface state into `final_verdict.json`. Universal — no per-topic logic. Wave 57 (2026-05-14): +50 for Slice 18 prose polish + accountability split + initial Phase G sidecar reconciliation.
