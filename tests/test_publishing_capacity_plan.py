...
def test_capacity_plan_includes_unique_topic_expansion_path() -> None:
    ...
    assert plan["unique_topic_expansion"] == {
        ...
        "intervention_projection_command": "python scripts/materialize_fact_topic_packs.py --strategy fact-intervention-cross --limit 4511",
        ...
        "next_strategy": "if grouped/field-cross projections create 0, run fact-pair-cross then fact-intervention-cross; persist only packs that pass peer specificity",
...
def test_live_plan_reports_http_topic_groups_materializer_access(tmp_path: Path, monkeypatch) -> None:
    topic_packs = tmp_path / "topic_packs"
    topic_db = tmp_path / "topic_packs_db"
    runs = tmp_path / "runs"
    topic_packs.mkdir()
    ...
    assert plan["bulk_materializer_access"]["status"] == "ok"
    assert plan["bulk_materializer_access"]["mode"] == "http_topic_groups"
    assert plan["bulk_materializer_access"]["http_env_names_checked"] == [
