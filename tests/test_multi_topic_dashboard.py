...
# ---------- Slice 8 step F: end-game funnel + cluster summary --------

def test_render_md_surfaces_full_endgame_funnel():
    """Topics that ran the end-game pipeline expose the 9-stage
    funnel with every transition visible."""
    s = _summary(
        topic="rapamycin",
        extraction_funnel={
            "retrieved": 583, "classified_keep": 428,
            "classified_drop": 155,
            "extracted_ok": 200, "extracted_cached": 6,
            "extracted_failed": 22, "spar_accepted": 11,
            "clustered_into_n": 4, "synthesized": 1,
        },
    )
    md = dash.render_md([s])
    assert "End-game funnel" in md
    # Each stage number visible
    for n in ("583", "428", "11", "4", "1"):
        assert n in md, f"missing {n} in dashboard render"


def test_render_md_surfaces_cluster_summary():
    """Cluster summary line shows shape of synthesis input."""
    s = _summary(
        topic="rapamycin",
        cluster_summary={
            "n_clusters": 4, "n_outcome_classes": 3,
            "n_design_types": 2,
            "n_top_receipts": 12, "n_total_receipts": 17,
        },
    )
    md = dash.render_md([s])
    assert "Clusters:" in md
    assert "4" in md  # n_clusters
    assert "3 outcome classes" in md
    assert "12" in md  # top
    assert "17" in md  # total


def test_render_md_omits_endgame_funnel_when_no_data():
    """Pre-Wave-7 topics with empty extraction_funnel skip the line."""
    s = _summary(
        topic="aspirin", extraction_funnel={}, cluster_summary={},
    )
    md = dash.render_md([s])
    assert "End-game funnel" not in md
    assert "Clusters:" not in md


def test_topic_summary_back_compat_defaults_empty():
    """TopicSummary's new fields default to {} so existing code
    paths that don't pass them keep working."""
    s = dash.TopicSummary(
        topic="x", n_runs=0, best_run_id="?", best_verdict="?",
    )
    assert s.extraction_funnel == {}
    assert s.cluster_summary == {}
