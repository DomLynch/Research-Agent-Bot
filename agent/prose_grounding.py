...
_PROMPT += " Packet bundle source_index values are original zero-based indexes; never infer them from list position. Displayed [bundle:N] citation labels are one-based: [bundle:1] maps to internal source_index 0, [bundle:2] to 1. Compare source identity after that conversion; a label differing numerically from its zero-based index is not itself an error. Reject a genuinely wrong source identity or unsupported citation. The source catalogue gives identities and classifications, not evidence for uncited effects. Uncited scientific effects must fail; uncited author scope/process requires author_context support. Review each statement only against its complete cited sources; catalogue totals do not establish whole-corpus effects."
...
_PROMPT += " If source_partition is present, this is one lossless subset of a large cited set. Judge this statement against every complete source supplied in this subset. If any clause needs an absent source for a comparison, return supported=false; do not infer its result from the catalogue. Every subset must pass before the original statement can be approved."
