...
_RESULT_SIGNAL_RE = re.compile(r"\b(?:lower(?:s|ed|ing)?|decreas(?:e[sd]?|ing)|improv(?:e[sd]?|ing|ement|ements)|increas(?:e[sd]?|ing)|reduc(?:e[sd]?|ing|tion|tions)|elevat(?:e[sd]?|ing|ion)|enhanc(?:e[sd]?|ing|ement)|attenuat(?:e[sd]?|ing|ion)|remain(?:s|ed|ing)?|unaffect(?:ed)?|unchanged|differ(?:ed|ence|ences)|associated|association)\b|\b(?:no|not|statistically)\s+significant\b|\bdid not (?:change|decrease|improve|increase|reduce)\b|\b(?:better|worse|greater|higher)\b.{0,80}\b(?:than|compared (?:with|to))\b", re.I)
...
def _add_source_trace_findings(paper_md: str, rows: Sequence[dict[str, Any]], ask: str) -> str:
    ...
    candidates: list[tuple[bool, int, bool, bool, int, str, str, str]] = []
    for number, row in enumerate(rows, 1):
        label, locator = _label(row), _stable_locator(row)
        if label and locator and row.get("thesis_text"):
            candidates.extend((number in used, depth, str(row.get("directness") or "").lower() != "direct", not str(row.get("evidence_tier") or "").upper().startswith("A"), number, label, locator, span) for depth, span in enumerate(_result_spans(row)))
    added = 0
    for _used, _depth, _indirect, _lower_tier, number, label, locator, span in sorted(candidates):
...
def _sentences(text: str) -> list[str]:
    ...
    protected = re.sub(r"(\b[Pp]\s*[<=>≤≥]\s*)\.(?=\s+\d)", lambda match: match[1] + _PROTECTED_PERIOD, protected)
